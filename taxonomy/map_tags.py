...
RULES = [
  ("bridge-sig-bypass", r"(zero.?hash|sigset|signature.?spoof|forged.*(proof|merkle|sig|deposit|withdrawal|burn)|quorum|verifier.*accept|mmr.*bound|ibc.*(forged|precompile)|crosschain.*(manager|create)|address.?collision|spv.*proof|finalize.*withdrawal|deposit.*msg.*unvalidated|3of.*multisig.*forged)"),
  ("replay-missing-idempotency", r"(replay|duplicate.?call|double.*(withdraw|claim|spend)|no.?epoch.?replay|emergencyBurn|position.?lock.*unlock|claim.*multiple|refund.*attacker)"),
  ("verify-gated-by-position", r"(batch.?position|%.*==.*0.*verify)"),
  ("oracle-spot-no-twap", r"(oracle|twap|vwap|flashloan.*(manipulat|misprice|price|inflate)|spot.*(pump|price|reserve|amm)|collateral.*inflat|price.?(feed|manipulat)|chainlink|misprice|stale.*price|perp.*oracle|thin.*liquidity|reserve.*oracle)"),
  ("reentrancy-readonly-and-callback", r"(reentrancy|read.?only|callback.*(mint|burn|before|reentrant|harvest|collect)|erc77|erc677|erc3525|erc1155|onerc.*received|hook.*reentr|sellshares.*reentr|routeprocessor.*callback|cook.*action.*reset|virtualprice)"),
  ("access-control-unprotected", r"(unprotected|missing.*(msg.?sender|check|initializer|caller)|unvalidated.*(input|router|calldata|market|origin|signer)|permissionless|forged.*approval|initialize.*(missing|flag|version)|admin.?key|backdoor|owner.*transfer|allowlist|whitelist|grant.*role|public.*callable|publicly.*(set|update)|selfswap|registerallowed|ownable.*transferownership|controllable.*transferfrom|enterprise.*approval)"),
  ("infinite-mint-unguarded", r"(infinite.?mint|dormant.?minter|deployer.*mint|missing.*burn|mint.*unlimited|no.?debt.*recorded|mint.*(dump|cap|bypass)|unregistered.*supply|cap.*bypass|print.*token|supply.*zero.*mint|bonding.*curve.*mint.*burn|cross.?rate.*mint)"),
  ("donation-share-inflation", r"(donat|exchangeRate.*round|pricePerShare|share.*inflat|totalSupply.*0|donation.*(recursive|bypass|inflate)|empty.*4626)"),
  ("overflow-rounding-invariant", r"(overflow|underflow|checked_shlw|rounding|fee.?divisor|k.?invariant|double.?decimal|debt.?share|double.?count|precondition.*violation|stableswap.*newton|solver.*underflow|linear.*pool.*round|near.?zero.*round)"),
  ("proxy-upgrade-reinit", r"(reinit|proxy.*upgrade|upgradeTo|slot.*collision|initialize.*flag|version.*bump|impl.*upgrade|upgrade.*impl|upgrade.*malicious|metamorphic|selfdestruct|delegatecall.*(malicious|owner)|storage.*overwrite|handler.*overwrite)"),
  ("governance-flashloan-vote", r"(flashloan.*governance|flashloan.*vote|no.?timelock|low.?vote.?lock|governance.*flash|emergency.*commit|abandoned.*governance)"),
  ("logic-invariant-break", r"(tokenIn.*tokenOut|self.?transfer.*balance|fee.?on.?transfer.*skew|comp.*scal|single.?token.?pool|cross.?rate|msg.?value.*confusion|order.*mismatch|accounting.*double|extrapolat|weight.?lag|vault.*event.*override|manual.*balance.*inflate)"),
]
...
out = {c: {"name": c, "source": SOURCE[c], "languages": LANGS[c], "detection_signature": SIG[c], "anchors": []} for c, _ in RULES}
residual = []
for r in data:
