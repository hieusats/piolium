/**
 * External-doc ("Tier-1") knowledge-base ingestion.
 *
 * Ported from vigolium-audit's `src/engine/knowledge-base.ts`. Ingests
 * user-supplied documentation (a `--plm-knowledge-base` path, `--plm-knowledge-base-raw`
 * string, or an auto-discovered `knowledge-base/` directory) into an immutable,
 * cited staging corpus under `piolium/attack-surface/knowledge-base-input/` that
 * the KB0 loader agent turns into `knowledge-base-seed.md`.
 *
 * The input is treated as **untrusted data, never instructions**. Every guard
 * from the upstream engine is preserved verbatim: symlink/realpath containment,
 * a TOCTOU-hardened bounded read, size/count/depth budgets, markdown-only
 * filtering, fatal UTF-8 + NUL rejection, and a sha256 aggregate manifest that
 * is re-verified on load. Only imports, on-disk path constants, and the two
 * optional couplings (resume + prior-run adoption) are re-homed onto piolium's
 * `audit-state.ts` and a local git helper.
 */
...
/** Output dir under the target repo. Excluded from doc discovery + git-clean check. */
const RESULTS_DIRNAME = "piolium";
...
const KB_INPUT_MANIFEST_PATH = "piolium/attack-surface/knowledge-base-input/manifest.json";
const KB_INPUT_CORPUS_PATH = "piolium/attack-surface/knowledge-base-input/corpus.md";
const KB_INPUT_SEED_PATH = "piolium/attack-surface/knowledge-base-seed.md";

/** Absolute results dir for a target repo (`<cwd>/piolium`). */
function resultsDirFor(targetDir: string): string {
	return join(targetDir, RESULTS_DIRNAME);
}

/**
 * Resolve + stage external KB input for a *consuming* audit mode (balanced /
 * deep), persisting the reference on the audit. Reads `PIOLIUM_KNOWLEDGE_BASE` /
 * `PIOLIUM_KNOWLEDGE_BASE_RAW`, an auto-discovered `knowledge-base/` dir, or —
 * when none — an adopted prior clean KB run. Returns `{}` (a clean no-op) when
 * nothing resolves, and swallows errors to a `{ error }` result so an optional
 * KB never fails the audit. The staged corpus is read by `knowledge-base-builder`.
 */
export async function ingestKnowledgeBaseForRun(args: {
	cwd: string;
	auditId: string;
	notify?: (text: string, level: "info" | "warning" | "error") => void;
}): Promise<{ reference?: KnowledgeBaseReference; error?: string }> {
	...
	try {
		...
		const reference = await stageKnowledgeBaseInput(resultsDirFor(args.cwd), resolved);
		await persistKnowledgeBaseReference(args.cwd, args.auditId, reference);
		args.notify?.(
			`Ingested knowledge base: ${reference.file_count} doc(s), staged at ${reference.corpus_path}.`,
			"info",
		);
		return { reference };
	} catch (err) {
		const message = errorMessage(err);
		args.notify?.(`Knowledge-base input ignored: ${message}`, "warning");
		return { error: message };
	}
}

/** Persist a run's staged knowledge-base reference onto its audit record. */
export async function persistKnowledgeBaseReference(
	cwd: string,
	auditId: string,
	...
	await mutateAuditState(cwd, (state) => ({
		...
		audits: state.audits.map((a) =>
			a.audit_id === auditId ? { ...a, knowledge_base: reference } : a,
		),
	}));
}

function knowledgeBaseReference(input: ResolvedKnowledgeBase): KnowledgeBaseReference {
	return {
		source_kind: input.sourceKind,
		source_label: input.sourceLabel,
		manifest_path: KB_INPUT_MANIFEST_PATH,
		corpus_path: KB_INPUT_CORPUS_PATH,
		seed_path: KB_INPUT_SEED_PATH,
...
export async function resolveKnowledgeBaseInput(args: {
	...
	/** Exact audit selected by the caller, when known. */
	...
	if (args.path !== undefined && args.raw !== undefined) {
		throw new Error("--plm-knowledge-base and --plm-knowledge-base-raw are mutually exclusive");
	}

	const resultsDir = resultsDirFor(args.targetDir);
	const resumeContext = args.resume
		? resolveResumableKnowledgeBaseContext(args.targetDir, args.resumeMode, args.resumeAuditId)
		: undefined;
	...
	if (args.resume) {
		...
		if (
			resumeContext.reference === undefined &&
			(args.path !== undefined || args.raw !== undefined)
		) {
			throw new Error(
		...
		if (staged === undefined || !referencesMatch(resumeContext.reference, staged)) {
			throw new Error(
				`cannot resume audit ${resumeContext.auditId}: the live staged knowledge base does not match the corpus recorded when that audit started`,
			);
		}
	}
...
function resolveResumableKnowledgeBaseContext(
	targetDir: string,
	mode?: AuditMode,
	auditId?: string,
): ResumableKnowledgeBaseContext | undefined {
	try {
		const state = readAuditState(targetDir).state;
		if (!state) return undefined;
		// A KB resume always pins the exact audit id; the by-mode lookup is the
		// fallback the resume-gauntlet tests exercise. (No caller omits both.)
		const audit =
			auditId !== undefined
				? state.audits.find(
						(candidate) =>
							candidate.audit_id === auditId &&
							(mode === undefined || candidate.mode === mode) &&
							candidate.status !== "complete",
					)
				: mode !== undefined
					? findResumableAuditByMode(state, mode)
					: undefined;
		if (audit === undefined) return undefined;
		return {
			auditId: audit.audit_id,
			reference: audit.knowledge_base,
		};
	} catch {
		return undefined;
	}
...
/** Latest resumable (in_progress preferred over failed) audit of a given mode. */
function findResumableAuditByMode(
	state: AuditStateFile,
	mode: AuditMode,
): AuditRunState | undefined {
	const sorted = [...state.audits]
		.filter((audit) => audit.mode === mode)
		.sort((a, b) => (a.started_at < b.started_at ? 1 : -1));
	return sorted.find((a) => a.status === "in_progress") ?? sorted.find((a) => a.status === "failed");
}

function referencesMatch(expected: KnowledgeBaseReference, staged: ResolvedKnowledgeBase): boolean {
	return (
		expected.aggregate_sha256 === staged.aggregateSha256 &&
...
/**
 * Temp-file-then-rename write. Copied from vigolium's `util.ts#atomicWrite` so
 * the ingestion engine stays a single self-contained module. A unique pid+uuid
 * staging suffix keeps concurrent writers from clobbering each other.
 */
async function atomicWrite(path: string, contents: string): Promise<void> {
	await mkdir(dirname(path), { recursive: true });
	const tmp = `${path}.plm-staging.${process.pid}.${randomUUID()}`;
	try {
...
/**
 * Recreate the durable input corpus. This deliberately does not write
 * knowledge-base-seed.md; the KB0 loader agent owns that semantic output.
 */
...
async function prepareStagingDirectory(resultsDir: string): Promise<{ inputDir: string }> {
	...
	if (existing?.isSymbolicLink()) {
		throw new Error(`refusing to stage knowledge-base input through a symbolic link: ${inputDir}`);
	}
...
async function assertRealpathContained(parent: string, child: string): Promise<void> {
	...
	if (
		...
		rel === ".." ||
		rel.startsWith(`..${sep}`) ||
		resolve(realParent, rel) !== realChild
	) {
...
async function resolveExplicitPath(value: string): Promise<ResolvedKnowledgeBase> {
	...
	try {
		...
		throw new Error(`--plm-knowledge-base: cannot read ${source}: ${(err as Error).message}`);
	}
	if (info.isSymbolicLink()) {
		throw new Error(`--plm-knowledge-base: symbolic links are not accepted (${source})`);
	}
	if (info.isFile()) {
		assertMarkdownPath(source, "--plm-knowledge-base");
	...
	if (!info.isDirectory()) {
		throw new Error(`--plm-knowledge-base: expected a markdown file or directory (${source})`);
	}
	const files = await collectMarkdownFiles(source, basename(source), 0, { files: 0, bytes: 0 });
	if (files.length === 0) {
		throw new Error(`--plm-knowledge-base: no .md or .mdx files found under ${source}`);
	}
...
function resolveRaw(raw: string): ResolvedKnowledgeBase {
	...
	if (raw.trim().length === 0) throw new Error("--plm-knowledge-base-raw must not be empty");
	if (bytes > MAX_RAW_BYTES) {
		throw new Error(
			`--plm-knowledge-base-raw is ${bytes} bytes, exceeds the ${MAX_RAW_BYTES}-byte cap; use --plm-knowledge-base with a directory instead`,
		);
	}
	if (raw.includes("\0")) throw new Error("--plm-knowledge-base-raw contains a NUL byte");
...
async function resolveAutoDiscovered(
	targetDir: string,
): Promise<ResolvedKnowledgeBase | undefined> {
...
async function collectMarkdownFiles(
	...
	if (depth > MAX_SOURCE_DEPTH) {
		throw new Error(
			`knowledge-base directory nesting exceeds ${MAX_SOURCE_DEPTH} levels (${current})`,
		);
	}
	let entries: Dirent[];
	try {
	...
	for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
		...
		if (entry.isDirectory()) {
			...
			files.push(
				...(await collectMarkdownFiles(path, join(logicalRoot, entry.name), depth + 1, budget)),
			);
...
async function readSourceFile(
	...
	if (info.size > perFileLimit) {
		throw new Error(
			`knowledge-base file ${path} is ${info.size} bytes, exceeds the ${perFileLimit}-byte per-file cap`,
		);
	}
	...
	try {
		...
		if (openedInfo.size > perFileLimit) {
			throw new Error(
				`knowledge-base file ${path} is ${openedInfo.size} bytes, exceeds the ${perFileLimit}-byte per-file cap`,
			);
		}
...
function buildResolved(
	...
	if (totalBytes > MAX_TOTAL_BYTES) {
		throw new Error(
			`knowledge-base is ${totalBytes} bytes, exceeds the ${MAX_TOTAL_BYTES}-byte total cap`,
		);
	}
...
export async function loadStagedKnowledgeBase(
	resultsDir: string,
): Promise<ResolvedKnowledgeBase | undefined> {
...
async function resolveExistingStagingDirectory(resultsDir: string): Promise<string | undefined> {
	const resolvedResults = resolve(resultsDir);
	...
	if (
		!(await ensureDirectoryWithoutSymlink(sourcesDir, false, "knowledge-base sources directory"))
	) {
		return undefined;
	}
	await assertRealpathContained(resolvedResults, attackSurfaceDir);
...
async function loadPriorKnowledgeBaseRun(
	targetDir: string,
): Promise<ResolvedKnowledgeBase | undefined> {
	const resultsDir = resultsDirFor(targetDir);
	let latest: AuditRunState | undefined;
	try {
		const state = readAuditState(targetDir).state;
		latest = state ? latestAudit(state) : undefined;
	} catch {
		return undefined;
	}
	if (latest?.mode !== "knowledge-base" || latest.status !== "complete") return undefined;
	const current = probeGitHead(targetDir);
	if (!current.available || !latest.commit || latest.commit !== current.commit) return undefined;
...
async function resolveCompletedKnowledgeBaseOutput(
	targetDir: string,
	auditId: string,
): Promise<ResolvedKnowledgeBase | undefined> {
	const resultsDir = resultsDirFor(targetDir);
...
// --- git helpers (re-homed from vigolium's git.ts, excluding piolium/) ---

/** Kill any git call that stalls (e.g. on an auth prompt), matching recon.ts. */
...
function probeGitHead(cwd: string): { available: boolean; commit: string | null } {
	if (runGit(["rev-parse", "--is-inside-work-tree"], cwd) !== "true") {
		return { available: false, commit: null };
	}
	const commit = runGit(["rev-parse", "HEAD"], cwd);
	return { available: commit !== null, commit };
}

/**
 * Whether the target's tracked and untracked source snapshot matches HEAD,
 * deliberately excluding `piolium/`. A `null` result means git could not
 * establish cleanliness and must be treated as unsafe for reuse.
 */
export function isGitWorktreeCleanForKnowledgeBaseReuse(cwd: string): boolean | null {
