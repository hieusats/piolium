...
1. `piolium/attack-surface/knowledge-base-report.md` — sections `## Attack Surface`, `## DFD/CFD Slices`, `## Architecture Model`, `## High-Risk CFD Slices`, and `## Commit Archaeology` (for HIGH-risk commits touching auth paths).
2. `piolium/codeql-artifacts/entry-points.json` if present (Phase 4 produces this; use it to cross-check that every framework route surfaces in your matrix).
...
Write `piolium/attack-surface/authz-matrix.md` with one row per endpoint:
...
Now that the matrix is built, produce the authoritative `piolium/attack-surface/unauthenticated-surface.md` — the exhaustive view of what an **anonymous attacker** (no valid session, token, or API key) can reach. Phase P3 `knowledge-base-builder` seeded a best-effort version of this file from the architecture model; **you supersede it** with the route-matrix-derived list because your enumeration is exhaustive. Read the existing file first (if present) and carry over any non-route entry points it captured that your matrix does not cover (e.g. an unauthenticated queue consumer the threat model noted).
...
For each finding class below, scan the matrix + source and emit a draft whenever the evidence meets the threshold. Write drafts to `piolium/findings-draft/p6-<NNN>-<slug>.md`.
...
Write `piolium/attack-surface/authz-coverage-gaps.md` listing endpoints that **you did not feel confident about** (Expected Scope = `unknown`, framework not detected, dynamic registration). Phase 10 chambers must review these manually.
...
Write each finding to `piolium/findings-draft/p6-<NNN>-<slug>.md` (NNN zero-padded, starting 001):
...
- Do NOT chase hypotheses that Phase 5 Deep Probe already recorded as VALIDATED for the same endpoint (check `piolium/probe-workspace/*/probe-summary.md` if it exists when you start — otherwise run your sweep normally and let the chamber deduplicate)
...
At the end of your run, append a short `## Authorization Audit` section to `piolium/attack-surface/knowledge-base-report.md`:
...
- Dynamic/unresolved endpoints: <M> (see `piolium/attack-surface/authz-coverage-gaps.md`)
- Drafts filed: <count> (split by class)
- Matrix: `piolium/attack-surface/authz-matrix.md`
- Unauthenticated surface: `piolium/attack-surface/unauthenticated-surface.md` (<P> pre-auth entry points, <Q> flagged missing-guard/middleware-gap)
```
