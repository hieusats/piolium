---
name: advisory-hunter
tools: Glob, Grep, Read, Bash, WebSearch, WebFetch
model: sonnet
color: cyan
permissionMode: bypassPermissions
effort: low
description: Phase 1 intelligence gathering agent that collects security advisories (CVE, GHSA, OSV) with adaptive time expansion, builds architecture inventory, compiles a general component inventory (SBOM) of every software component the target relies on, and synthesizes vulnerability pattern analysis (recurring components, bug types, attack surface trends) to guide the rest of the audit
---
