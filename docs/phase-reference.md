...
| `Q2` | Fast Static Analysis | Runs a tight, high-signal static pass for issues such as command injection, traversal, SSRF, broken auth, hardcoded crypto, and weak authn/z; best-effort enumeration of the unauthenticated (pre-auth) surface and one-band severity elevation for findings reachable from it. | `piolium/attack-surface/lite-q2-summary.md`; `piolium/attack-surface/unauthenticated-surface.md`; `piolium/findings-draft/q2-<NNN>-<slug>.md` |
...
| `L2` | Architecture & Threat Model | Builds a compact knowledge base with project type, trust boundaries, DFD/CFD slices, attack modes, coverage gaps, and the unauthenticated (pre-auth) surface (final — balanced has no authz-auditor phase). | `piolium/attack-surface/knowledge-base-report.md`; `piolium/attack-surface/unauthenticated-surface.md` |
...
| `P3` | Architecture & Threat Model | Builds the deep knowledge base and reusable entry point inventory; seeds the unauthenticated (pre-auth) surface (superseded by `P5`). | `piolium/attack-surface/knowledge-base-report.md`; `piolium/attack-surface/architecture-entrypoints.md`; `piolium/attack-surface/unauthenticated-surface.md` (seed) |
...
| `P5` | Authorization & Access Control | Enumerates public routes or operations and records expected versus actual authorization checks; supersedes the `P3` seed with the exhaustive matrix-derived unauthenticated (pre-auth) surface. | `piolium/attack-surface/public-routes-authz-matrix.md`; `piolium/attack-surface/unauthenticated-surface.md` (final); `piolium/findings-draft/p5-<NNN>-<slug>.md` |
