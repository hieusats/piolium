...
# All phase outputs write into piolium/attack-surface/knowledge-base-report.md (phases 1-9)
# or piolium/final-audit-report.md (phase 15). Checks verify KB sections and CodeQL artifacts.
#
...
# Files that may legitimately appear inside piolium/attack-surface/ — anything else there is an orphan.
...
# Files that must NOT exist in piolium/ (consolidated into knowledge-base-report.md)
STALE_FILES = [
    "advisory-hunter-report.md",
...
def check_adversarial_reviews(security_dir: str) -> tuple[bool, str]:
    ...
    if not os.path.isdir(reviews_dir):
        return False, (
            "CRITICAL/HIGH VALID findings exist but piolium/adversarial-reviews/ is missing. "
    ...
    if not review_files:
        return False, (
            "CRITICAL/HIGH VALID findings exist but piolium/adversarial-reviews/ is empty. "
            "P11-LITE Stage 2 cold verification is required for CRITICAL and HIGH findings."
        )
...
def lint_all(security_dir: str) -> tuple[bool, list[str]]:
    """Full-audit consistency checks across the piolium/ directory."""
    import json
    ...
    if os.path.isfile(final_report):
        ...
        if os.path.isdir(findings_dir):
            ...
            for fid in set(ids_in_report):
                ...
                if not matches:
                    errors.append(
                        f"final-audit-report.md references {fid} but no matching directory in piolium/findings/"
                    )
        ...
        for fname in os.listdir(draft_dir):
            ...
            if "Verdict: VALID" in content:
                ...
                if os.path.isdir(findings_dir):
                    ...
                    if not matches:
                        errors.append(
                            f"findings-draft/{fname} has Verdict: VALID but was not promoted to piolium/findings/"
                        )
    ...
    if os.path.isdir(findings_dir):
        for entry in sorted(os.listdir(findings_dir)):
            ...
            if not os.path.isfile(report_path):
                errors.append(
                    f"findings/{entry}/report.md is missing — finding-reporter "
    ...
    # 7. Orphan detection: files in piolium/ root not in the known output set.
    ...
    for entry in os.listdir(security_dir):
        ...
        if os.path.isdir(entry_path):
            if entry not in known_dirs:
                errors.append(f"Unexpected directory in piolium/: {entry}/")
        elif entry.endswith(".md") or entry.endswith(".json"):
            if entry not in known_outputs:
                errors.append(f"Orphaned file in piolium/ (not in any phase output map): {entry}")
    ...
    if os.path.isdir(attack_surface_path):
        for entry in os.listdir(attack_surface_path):
            ...
            if os.path.isdir(sub):
                errors.append(f"Unexpected directory in piolium/{ATTACK_SURFACE_DIR}/: {entry}/")
            elif entry not in KNOWN_ATTACK_SURFACE_FILES:
                errors.append(f"Orphaned file in piolium/{ATTACK_SURFACE_DIR}/: {entry}")
